...
Dynamite::Dynamite(const char* file_path, const char* file_path_worm)
{
    ...
    if (image == NULL)
        cout << "Error loading Dynamite image." << endl;
}

int Dynamite::getDamage(int power)
{
    switch (power)
    {
    case 1:
...
int Dynamite::getDiameter(int power)
{
    switch (power)
    {
    case 1:
...
float Dynamite::getSpeed(int power)
{
    ...
    return 7.3 * percentage_modifier;
}

float Dynamite::getExplosionBias()
{
    return 0;
}

bool Dynamite::readyToShoot(void)
{
    if (Shots.size() > 0)
        return true;
    else
...
bool Dynamite::Shoot(Cmap* map, Cworm* worm)
{
...
void Dynamite::createShotW(int angle, int power, Point start, int fuse, int ID)
{
    Shot temp_Shot;
...
bool Dynamite::checkExplosion(void)
{
    return false;
}
