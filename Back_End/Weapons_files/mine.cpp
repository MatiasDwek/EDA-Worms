...
Mine::Mine(const char* file_path, const char* file_path_worm)
{
    ...
    type = FUSE;
    past_refresh = 0;
    explosion_sound = al_load_sample("./Sounds/explosion2.wav");
    number.x = 1;
    number.y = 1;
    ...
    if (image == NULL)
        cout << "Error loading Mine image." << endl;
}

int Mine::getDamage(int power)
{
    return 35 + 5 * power;
}

int Mine::getDiameter(int power)
{
    switch (power)
    {
    case 1:
...
float Mine::getSpeed(int power)
{
    ...
    return 4 * percentage_modifier;
}

float Mine::getExplosionBias()
{
    return 0;
}

bool Mine::readyToShoot(void)
{
    if (Shots.size() > 0)
        return true;
    else
...
bool Mine::Shoot(Cmap* map, Cworm* worm)
{
...
void Mine::createShotW(int angle, int power, Point start, int fuse, int ID)
{
    Shot temp_Shot;
