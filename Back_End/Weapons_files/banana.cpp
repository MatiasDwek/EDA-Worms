...
Banana::Banana(const char* file_path, const char* file_path_worm)
{
    ...
    release_sound = al_load_sample("./Sounds/bananaimpact.wav");
    fragments = al_load_bitmap("./Images/Weapons/banana/sbanana.png");
    number.x = 3;
    number.y = 0;
    ...
    if (image == NULL)
        cout << "Error loading Banana image." << endl;
}

int Banana::getDamage(int power)
{
    switch (power)
    {
    case 1:
...
int Banana::getDiameter(int power)
{
    switch (power)
    {
    case 1:
...
float Banana::getSpeed(int power)
{
...
float Banana::getExplosionBias()
{
    return 0;
}

int Banana::number_of_fragments(int power)
{
    switch (power)
    {
    case 1:
...
bool Banana::Shoot(Cmap* map, Cworm* worm)
{
    ...
    if ((al_get_timer_count(time_to_explosion) >= (i->fuse * 10)) && (!Shots.empty()))
    {
        ...
        for (int ii = 0; ii < fragments; ii++, angle += 360.0 / fragments)
...
bool Banana::Shoot_fragments(Cmap* map, Cworm* worm)
{
    float t = 0;
    float j;
    float temp_x;
...
void Banana::createShotW(int angle, int power, Point start, int fuse, int ID)
{
    _power = power;
...
bool Banana::checkExplosion(void)
{
    return false;
}
